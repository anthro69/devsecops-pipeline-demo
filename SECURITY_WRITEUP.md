...
- Додати gate і на dependency-рівні: падати, якщо Safety або pip-audit
  знайшли CVE з CVSS ≥ 7 (critical/high).
- Виправити всі 7 знайдених вразливостей у гілці `fix/security-issues` і
  показати "до/після" в pipeline — наочна демонстрація ефективності гейту.
- Додати secret-scanning (наприклад, Gitleaks) окремим job'ом — hardcoded
  secret key (VULN-01) якраз це б підсвітив ще на етапі коміту.
- Додати `SBOM`-генерацію (наприклад, `cyclonedx-py`) як окремий
  артефакт pipeline для повної прозорості складу залежностей.
